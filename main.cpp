...
/*FUNCTION TO CALCULATE TURBULENT FLOW's HEAD LOSS FOR HORIZONTAL SECTION OF PIPE OF LENGTH L*/
...
ld local_head_loss(ld flow_velocity, ld fric_coefficient_kl)
{
    ...
    /* ----------------------------------------> NOTE NOTE NOTE <-------------------------------------------
        ...
        1.)Enlargement --->  Kl = pow(1-(a1/a2),2) 
        2.) Contraction -->  A1 = x*A2   x<1 for contraction
...
ld pump_power(ld tot_head, ld density_new, ld pump_rate){
    return 8*(pump_rate* density_new* 9.81* tot_head)/(3.6*pow(10, 6));
}
        ...
        do {
            ...
            switch (choice)
            {
                ...
                case 0:
                {
                    ...
                    cin>>pump;
                    cout<<"\n\nEXCESS PUMP POWER BEING USED = "<< pump - abc<<endl;
                    break;
                }
